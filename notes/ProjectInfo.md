...
1. **Quarkus**: Cloud-native Java framework for fast startup, low memory usage, and Kubernetes-friendly microservices.
2. **Quarkus REST (JAX-RS)**: RESTful API framework used by service endpoints and the gateway routing layer.
3. **Hibernate ORM with Panache/JPA**: Persistence layer used for relational data access and entity management.
4. **SmallRye Reactive Messaging**: Event-driven messaging framework used with Kafka.
5. **Eclipse MicroProfile Config**: Externalized configuration support for environment-driven service settings.
6. **Jakarta CDI**: Dependency injection and lifecycle management for application components.
7. **MapStruct**: Compile-time DTO and entity mapping.
...
2. **JAX-RS Request Filters**: API gateway token checks and request security controls.
...
1. **SmallRye Kafka Connector**: Kafka integration for reactive messaging channels.
...
5. **Quarkus Test Utilities**: Framework support for service and integration testing in Quarkus applications.
...
4. **Java HttpClient**: HTTP client used for direct service integrations such as Keycloak calls.
